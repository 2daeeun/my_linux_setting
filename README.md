...
* OBS Studio & FFmpeg
```swift
sudo apt-get install ffmpeg && sudo apt-get install obs-studio -y
```
* Zoom 
```swift
...
* unzip 한글 깨짐 방지
```swift
sudo git clone https://github.com/zsh-users/zsh-syntax-highlighting.git ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-syntax-highlighting && sudo git clone https://github.com/zsh-users/zsh-autosuggestions ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-autosuggestions && mv .zshrc .zshrc.bak && wget -O .zshrc https://raw.githubusercontent.com/2daeeun/my_linux_mint_setting/master/zshrc
```
